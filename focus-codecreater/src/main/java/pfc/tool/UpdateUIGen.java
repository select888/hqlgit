...
public class UpdateUIGen  extends CommonGen implements InterGen {
	...
	public String gen(String beanname) {
		...
		for(String columnname:cmap.keySet()){
			if(cmap.get(columnname).equals("java.lang.String")||cmap.get(columnname).equals("java.lang.Double")
				...
				append(content, "				<input name=\""+columnname+"\" type=\"text\" maxlength=\"150\" th:value=\"${entity."+columnname+"}\"/>",1);
				append(content, "			</p>",1);
		...
		for(String joincolumnname:joinmap.keySet()){
				...
				append(content, "               <input name=\""+joincolumnname.toLowerCase()+"."+joinkey+"\" type=\"hidden\" th:value=\"${entity."+joincolumnname+"==null}?'':${entity."+joincolumnname+"."+joinkey+"}\"/>",1);
				append(content, "               <input name=\""+joincolumnname.toLowerCase()+"."+joincolumn+"\" type=\"text\" th:value=\"${entity."+joincolumnname+"==null}?'':${entity."+joincolumnname+"."+joincolumn+"}\"/>",1);
